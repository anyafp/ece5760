...
fix28 times_rho(fix28 a, fix28 new_rho);
fix28 times_rho(fix28 a, fix28 new_rho) {
   return multfix28(a,new_rho);
}
...
int main(void)
{
  ...
  fix28 max_rho = float2fix28(0.49);
	...
	while(1){	
		...
		while (((*audio_fifo_data_ptr>>24)& 0xff) > 1) {
			...
			for (i=1; i<drum_size-1; i++){
				for (j=1; j<drum_size-1; j++){
					//new_drum_temp = multfix28( (drum_n[i-1][j] + drum_n[i+1][j] + drum_n[i][j-1] + drum_n[i][j+1] - times4pt0(drum_n[i][j])), rho_0);
          new_drum_temp = multfix28( (drum_n[i-1][j] + drum_n[i+1][j] + drum_n[i][j-1] + drum_n[i][j+1] - times4pt0(drum_n[i][j])), rho_eff);
      ...
      //printf("drum = %f\n", fix2float28(drum_n[drum_middle][drum_middle]));
